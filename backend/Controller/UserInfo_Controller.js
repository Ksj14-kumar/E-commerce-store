...
const { isValidInfo } = require("./Controller/Userinfo_controller.helper")
class Info_Class {
    async Register(req, res) {
        try {
            ...
            savenewUser.save(async (err, result) => {
                if (err) {
                    const returnMessage = isValidInfo(err)
                    console.log(`🚀 ~ file: UserInfo_Controller.js:44 ~ Info_Class ~ savenewUser.save ~ returnMessage:${returnMessage}`)
                    return res.status(400).json({ message: returnMessage })
                }
                else {
                    console.log("🚀 ~ file: UserInfo_Controller.js:47 ~ Info_Class ~ savenewUser.save ~ result:", result)
            ...
            console.log("🚀 ~ file: UserInfo_Controller.js:67 ~ Info_Class ~ Register ~ err:", err)
